...
st.set_page_config(page_title="团队平均能力", page_icon="🙌")
st.sidebar.header("不同团队内成员的平均情况")
#读取数据
#读取数据
...
#定义雷达图维度及相应数据
df_tech = df_mean[['产品设计','业务分析','产品运营','项目管理','前端开发','安全体系','网络攻防']]
#df_business = df[['权益投资','固收投资','指数投资','量化投资','合规风控','风险绩效','研究管理']]
#df_diathesis = df[['灵气','逻辑性','创新思维','价值导向','激情','情商','团队合作','承压能力','领导力']]
df_business = df_mean[df_view.query('维度 == "业务知识"').loc[:,'指标'].values.tolist()]
df_diathesis = df_mean[df_view.query('维度 == "通用素质"').loc[:,'指标'].values.tolist()]
